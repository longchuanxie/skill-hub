...
function checkDescriptionQuality(description: string): {
  valid: boolean;
  issues: string[];
  severity: 'low';
} {
...
function checkSkillStructure(skillData: any): {
  valid: boolean;
  issues: string[];
  severity: 'medium';
} {
...
function checkPromptStructure(promptData: any): {
  valid: boolean;
  issues: string[];
  severity: 'medium';
} {
...
export async function reviewSkill(skillData: any, filePath?: string): Promise<AutoReviewResult> {
  ...
  if (filePath && !contentReviewConfig.skipFormatValidation) {
    try {
      const validationResult = await validateSkillUpload(
        filePath,
        path.join(process.cwd(), 'temp', `review-${Date.now()}`),
      );
      ...
      reasons.push(
        `ZIP包验证过程中发生错误: ${error instanceof Error ? error.message : '未知错误'}`,
      );
      severity = maxSeverity(severity, 'high');
    }
  }
